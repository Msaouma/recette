...
const Details = ({route, navigation}) => {
  const { recetteId, setRecetteId } = route.params;
  const { imageRecette, SetImageRecette } = route.params;
  const [recDetail, setRecDetail] = useState();

  useEffect(() => {
    console.log('detail');
    axios.get(`https://api.spoonacular.com/recipes/${recetteId}/ingredientWidget.json?apiKey=f70834573a7c401f86f1b5a7a5c7d1f4`)
    .then(res => {
        const recettes = res.data.ingredients;
        setRecDetail(recettes);
    });    
},[])
const renderDetailItem = (itemData) => {
    return (
      <DetailsDes
        ...
        image={imageRecette}
        amount={itemData.item.amount.metric.value}
        unit={itemData.item.amount.metric.unit}
      />
    );
};
